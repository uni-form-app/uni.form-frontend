...
export default function Pedidos() {
  ...
  const [pedidos, setPedidos] = useState(meusPedidos)

  const confirmarRecebimento = (pedidoId: number) => {
    setPedidos(
      pedidos.map((pedido) =>
        pedido.id === pedidoId
          ? { ...pedido, status: "entregue", dataEntrega: new Date().toLocaleDateString() }
          : pedido,
      ),
    )
    ...
    toast({
      ...
      description: "O recebimento do produto foi confirmado com sucesso.",
    })
  }
                    ...
                    <div className="flex flex-col sm:flex-row gap-4">
                      ...
                      <div className="flex-1">
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-2">
                          <div>
                            ...
                            <h3 className="font-medium">
                              <Link href={`/produtos/${pedido.produto.id}`} className="hover:underline">
                                {pedido.produto.nome}
                              </Link>
                            </h3>
                          </div>
                        ...
                        <div className="mt-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                          ...
                          <Button onClick={() => confirmarRecebimento(pedido.id)} className="sm:self-end">
