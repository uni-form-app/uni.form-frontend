      ...
      <CardContent className="p-6">
        ...
        <div className="flex gap-4">
          <div className="relative w-20 h-20 rounded overflow-hidden shrink-0 bg-gray-100">
            <img
              src={`http://localhost:8080/public/${cart.ProductImages[0]?.path ?? ""}`}
              alt={cart.name}
            ...
            <div className="flex justify-between">
              <div>
                <h3 className="font-medium">
                  <Link to={`/product/${cart.id}`} className="hover:underline">
                    {cart.name}
                  </Link>
                </h3>
              </div>
              <div className="text-right">
                <div className="font-bold">R$ {cart.price.toFixed(2)}</div>
                <Button
                  variant="ghost"
                  size="sm"
